...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0: # 提示层数大于0
                if not self.text_layer: # vision layer
                    if not (counter > len(compound_prompts_deeper) - 1): # 2
                        ...
                        pmt_feature = compound_prompts_deeper[counter] # 2 768
                        pmt_feature = pmt_feature.expand(x.shape[1], -1, -1).permute(1, 0, 2) # 64 4 768
                        # Add the learnable tokens of this layer with the input, replaced by previous layer learnable tokens
                        x = torch.cat([prefix, pmt_feature], dim=0)
            ...
            if len(compound_prompts_deeper) > 0: # 提示层数大于0
                if self.text_layer: # text layer
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        prefix = x[:1, :, :]                                  # 保留文本特征的第一个词
                        suffix = x[1+self.compound_prompt_nctx:, :, :]        # 保留文本特征的后K个词
                        pmt_feature = compound_prompts_deeper[counter]        # 4 512 1-4 位置放置提示
                        pmt_feature = pmt_feature.expand(x.shape[1], -1, -1).permute(1, 0, 2)  # 64 4 512
                        # Add the learnable tokens of this layer with the input, replaced by previous layer learnable tokens
                        x = torch.cat([prefix, pmt_feature, suffix], dim=0)
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, design_details):
        ...
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width))
        ##
        self.fMRI_positional_embedding = nn.Parameter(scale * torch.randn(117, width)) # 分类
        # self.fMRI_positional_embedding = nn.Parameter(scale * torch.randn(83, width)) # 预测
        ##
        self.ln_pre = LayerNorm(width)
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        # x = self.conv1(x)                          # shape = [*, width, grid, grid]
        # x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        # x = x.permute(0, 2, 1)                     # shape = [*, grid ** 2, width]  batch_size 116 768
        # x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
        
        x = torch.cat([self.class_embedding.half() + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
        x = x + self.fMRI_positional_embedding.to(x.dtype)                               # batch_size 117 768

        # After positional embeddings, we will attach prompts with the model.
        if self.VPT_shallow:
        ...
        # Normal code as before
        x = self.ln_pre(x)      # batch_size 121 768
        x = x.permute(1, 0, 2)  # NLD -> LND 121 batch_size 768
        ...
        x = x.permute(1, 0, 2)  # LND -> NLD
        ##
        x_h = self.ln_post(x[:, 0, :])
        x_f = self.ln_post(x[:, 0:117, :]) # 1:117
        # x_f = torch.mean(x_f, dim=2).unsqueeze(2)
        # x_h = self.ln_post(x[:, 0, :])
        # x_f = self.ln_post(x[:, 0:83, :]) # 1:83
        ##
        if self.proj is not None:
            x_h = x_h @ self.proj
